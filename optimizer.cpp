...
int main(int argc, char* argv[]) {
    ...
    long long CACHE_size = atoi(argv[2]);         //获取cache大小

    int CACHE_int = CACHE_size / 8;         //计算可分配的int数量

    int Block_size = CACHE_int / p;         //计算分块大小
    int Block_num = size / Block_size;      //计算分块数量
    int Block_remain = size % Block_size;   //计算无法整除的部分
